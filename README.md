...
9. **건물 진입** — 학교 / 카페 / 집의 정문 영역에 들어서면 자동으로 실내 씬으로 전환됩니다. 안에서는 "나가기 ↓" 표시가 있는 출입구 쪽으로 걸어가면 다시 마을로 나옵니다. 시간대에 따라 실내에 있는 NPC 가 다릅니다 (예: 평일 오전 = 학교 교실에 마도카/히카루/만타).
10. **호감도 + 스토리** — 마도카와 호감도 60 이상이 되면 자동으로 회상 이벤트 발생. 다른 캐릭터에게도 비슷한 이벤트가 있습니다.
...
- [x] Phase A: 기능 정합성 (issues #18 #19) — 건물 진입 가능, 토요일 10:30 시작, NPC in-range 하이라이트, 인트로 후 안내 toast
- [x] Phase B: 비주얼 (issues #20 #21 #22) — 코드 기반 SD 캐릭터 스프라이트, 마을 디테일 (학교/집/카페/공원 + 길/풀), DialogBox 캐릭터 초상화 + 감정 표정
- [x] Phase C: 실내 씬 (issue #23) — 학교/카페/집 진입 시 indoor 씬 전환, 일과에 맞는 NPC 가 실내에 등장, 세이브 v2 round-trip
- [x] Phase QA-2: 최종 통합 테스트 (issue #24)
